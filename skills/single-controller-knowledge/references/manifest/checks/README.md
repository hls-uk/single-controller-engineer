...
network access. Copy this directory and `knowledge-manifest.schema.json`
together; each entrypoint resolves the schema from their shared parent.
...
The provenance check validates JSON records in the declared events directory,
rejects unknown record keys and invalid links, and asks local Git whether each
full landed object identifier is an ancestor of `HEAD`. An empty initialized
events directory is valid.
