...
3. One to three genuinely independent units are dispatched to isolated
   worktrees with packets generated by `sce harness-packet`. A unit composed
   on an older integration head refreshes before its first dispatch, which
   fast-forwards its empty branch and worktree so the packet binds the base
   the worker actually starts on. A unit's diff against its base must fit
...
4. Each frozen candidate gets a fresh adversarial frontier review bound to
   ...
   into `sce candidate-digest`, which prints the packet's domain-separated
   `candidateDiffHash`; a plain `sha256sum` of the same bytes never equals
   it. Before a unit's first candidate observation, and again after a sibling
   lands, the controller refreshes it onto the current integration head
   (`refresh-candidate`), which rebases the unit branch in its own worktree
   and discards every binding to the old base. Once a unit has landed (or is
   cancelled, parked, failed, or timed out), `close-unit` releases its path
   reservation and closes it.
