...
3. One to three genuinely independent units are dispatched to isolated
   worktrees with packets generated by `sce harness-packet`. A unit's diff
   against its base must fit 65,536 bytes, the reviewer packet's
   `candidateDiffByteCount` bound; a candidate that measures past it is
   refused with its measured size and repaired smaller, never reviewed.
