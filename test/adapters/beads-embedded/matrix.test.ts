  ...
  try {
    ...
    for (const [index, malformed] of malformedDeltas.entries()) {
      ...
      await executable(fakeDolt, [
        ...
        // The effect commit is the one slot-touching edge; every other range
        // the proof asks about is slot-untouched journal lineage.
        `    *"dolt_diff('${beforeHead}', '${effectHead}', 'issues')"*) printf '{"rows":[{"touched":1}]}' ;;`,
        `    *"FROM dolt_diff("*) printf '{"rows":[{"touched":0}]}' ;;`,
