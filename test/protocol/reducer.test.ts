...
for (const profile of ["local-ff", "remote-ff"] as const)
  for (const reason of [
    "integration_checkout_dirty",
    "integration_checkout_foreign",
  ] as const)
    test(`${profile} ${reason} keeps the unit approved`, () => {
      let approved = approvedCandidate("integrate", profile);
      if (profile === "remote-ff") {
        approved = stepUnit(approved, "unit-1", "publish_intent");
        approved = observeUnit(
          approved,
          "unit-1",
          "publish_observed",
          "publish",
      ...
      assert.equal(
        refused.effectJournal.find((entry) => entry.kind === "integrate")
          ?.status,
        "observed",
      );
      if (profile === "local-ff") {
        // The controller fixes the checkout and re-issues the same intent.
        assert.deepEqual(
          legalActions(refused)
            .filter(
              (action) => action.unitId === unitId && action.mode === "emit",
            )
            .map((action) => action.type)
            .filter(
              (type) =>
      ...
      assert.equal(
        reduce(
          ...
          event(
            ...
            "integrate_refused",
            {
              effectId: refused.effectJournal.find(
                (e) => e.kind === "integrate",
              )!.effectId,
              effectKind: "integrate",
              observationHash: HASH,
      ...
      assert.equal(
        reduce(
          ...
          event(
            ...
            "integrate_refused",
            {
              effectId: intended.effectJournal.find(
                (e) => e.kind === "integrate",
              )!.effectId,
              effectKind: "integrate",
              observationHash: HASH,
