...
export function previewFeedback(packet: FeedbackPacket): FeedbackPreview {
  return {
    targetUrl: "https://github.com/hls-uk/single-controller-engineer",
    repositoryId: packet.target.repositoryId,
    title: packet.title,
    body: packet.body,
    marker: packet.marker,
    requiresCurrentUserAuthority:
      packet.narrative !== undefined || packet.narrativeFindings.length > 0,
    narrativeFindings: packet.narrativeFindings,
  };
}
...
export function prepareFeedback(
  ...
  if (!isFeedbackSchema<SafeTelemetryInput>(SafeTelemetryInputSchema, input))
    return undefined;
  const toolVersion = normalizedSingleLine(input.toolVersion, 80);
  const toolchain = normalizedSingleLine(input.toolchain, 80);
  const stableErrorCode = normalizedSingleLine(input.stableErrorCode, 80);
  const capabilityId = normalizedSingleLine(input.capabilityId, 160);
  if (
